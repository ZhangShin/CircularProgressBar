...
/**
 * Created by zx on 2017/4/25.
 */

public class CircleView extends View {
    private static final String TAG = "CircleBar";

    private RectF mColorWheelRectangle = new RectF();//控件矩形范围
    ...
    private int mWheelColor = getResources().getColor(R.color.circle);//默认圆环颜色
    ...
    private void init(AttributeSet attrs, int defStyle) {
        //初始化一些值
        circleStrokeWidth = dip2px(getContext(), 8);
        ...
        mDefaultWheelPaint.setColor(getResources().getColor(R.color.background));
        ...
        textDesPaint.setColor(getResources().getColor(R.color.colorPrimaryDark));
    ...
    protected void onDraw(Canvas canvas) {
        RectF mCircleRectangle = new RectF();//圆圈的矩形范围
        mCircleRectangle.set(circleStrokeWidth + pressExtraStrokeWidth, circleStrokeWidth + pressExtraStrokeWidth,
                mColorWheelRadius*2+circleStrokeWidth, mColorWheelRadius*2);
        canvas.drawArc(mCircleRectangle, -180, 180, false, mDefaultWheelPaint);//画外接的圆环
        canvas.drawArc(mCircleRectangle, -180, mSweepAnglePer, false, mColorWheelPaint);//画圆环
